  ...
  if (a(e)) {
    ...
    if (G(a2, k2)) {
      let n = a2.replaceAll(k2, (l2) => {
        let c2 = l2.replace(/{|}/g, "").split(".").filter((m2) => !s3.some((d) => G(m2, d)));
...
function de(e, t = {}) {
  ...
  for (; o.length; ) {
    ...
    for (let u in m2) {
      let g2 = m2[u], f = ve(g2), p3 = G(u, a2) ? Q(d) : Q(d, re(u));
      if (i(f)) o.push({ node: f, path: p3 });
      else {
        let y2 = ae(p3), R = Y2(f, p3, s3, [a2]);
        Re(l2, y2, R);
        let T = p3;
...
var b2 = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  let a2 = function(l2, o = {}, c2 = []) {
    ...
    Object.entries(l2).forEach(([m2, d]) => {
      ...
      i(d) ? n(d, u, g2) : (i2[u] || (i2[u] = { paths: [], computed: (f, x2 = {}, p3 = []) => {
  ...
  if (s2(t)) {
    ...
    if (o) {
      ...
      i(o) && (c2.name = m(o.name, { name: e, type: s3 })), s2(c2.name) && (t = C2(`@layer ${c2.name}`, t), a2 == null || a2.layerNames(c2.name));
    }
    return t;
  }
  return "";
